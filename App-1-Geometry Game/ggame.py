...
def random_rect(initial_lower: int = 0, difference: int = 9):
    ...
    lower_left = Point(randint(initial_lower, initial_lower + difference),
                       randint(initial_lower, initial_lower + difference))
    upper_right = Point(randint(initial_upper, initial_upper + difference),
...
if __name__ == '__main__':
    ...
    print(
        f'Your point was inside the  rectangle : {user_point.point_falls_in_rect(rect1)}'
    )
    ...
    if consent == 'y' or consent == 'Y':
        ...
        if guessed_area == rect1.area():
            print("Congo!! You guessed correct area!")
        else:
            print(
                f"Good luck for next time.\nWell correct area is {rect1.area()}"
            )
